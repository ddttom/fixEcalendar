      ...
      try {
        ...
        try {
          ...
          if (!options.skipMerge && totalAdded > 0) {
            ...
            try {
              // Dynamic import of merge utility from src/scripts
              const mergePath = path.join(__dirname, 'scripts', 'merge-overlapping-events');
              ...
              console.log('You can manually run: npx ts-node src/scripts/merge-overlapping-events.ts');
            }
          } else if (options.skipMerge) {
            console.log('\n⏭️  Skipped automatic merge (--skip-merge specified)');
            console.log('To merge overlapping events, run: npx ts-node src/scripts/merge-overlapping-events.ts');
          }
